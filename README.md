...
| 수동 추가 | OCR 없이 동일 필드 입력 (티 컬러 탭 포함) |
| 상세 | 티 컬러 칩, 전반/후반, 동반자, 18홀, OUT/IN/TOTAL |
| 기록 | 검색·필터 (전체/내 기록/예시) |
| 통계 | 평균, 베스트/워스트, 추세, 코스별 평균 |


**티 컬러** (`teeColor`): `white` | `blue` | `red` (UI: 화이트 / 블루 / 레드).  
기본값은 **`blue`(블루)** — 국내 남성 레귤러 티로 가장 흔한 설정입니다. 레거시 IndexedDB 행도 hydrate 시 `blue`로 채웁니다. OCR이 화이트/블루/레드(또는 W/B/R) 티 표기를 찾으면 자동 선택하고, 못 찾으면 기본값을 편집 가능하게 둡니다.
